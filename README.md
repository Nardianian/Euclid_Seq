...
C) The MIDI stream for each rhythm is generated based on various independent commands or rotary knobs, which in order on the GUI are:
   ...
   C1) Note: to decide the note to play (the implementation of the possibility of choosing scales and/or chords is also planned).
...
D) ARP, the MIDI stream thus generated enters the arpeggiator:
   ...
   D2.1: Using the tickable popup menu with "Note" snapshot, user can view the incoming notes (from the MIDI stream of the rhythm created so far) and can select up to seven notes with their respective octaves (e.g., 
   ...
   D2.2: Use the "Mode" box/menu to select the mode (UP, DOWN, UP-DOWN, RANDOM) for arpeggiating the notes selected in the previous step (Arp-Notes).
